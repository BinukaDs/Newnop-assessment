...
class TaskService {
    async createTask(req) {
        let { title, description, priority, status, dueDate, assignedTo } = req.body;
    ...
    async getTaskById(taskId, user) {
        ...
        const task = await TaskModel.findById(taskId).populate("assignedTo", "_id username email");
        if (!task) {
    ...
    async getAllTasks(user, filters) {
        ...
        if (filters.search) {
            const safeSearchString = filters.search.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
            query.title = {
        ...
        tasks = (await TaskModel.find(query).populate("assignedTo", "username email"));
        if (!tasks) {
